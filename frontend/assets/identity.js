...
(() => {
  ...
  const run = () => {
    ...
    const face = p.avatar
      ...
      : '<span style="width:30px;height:30px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#D92D3F,#D92D3F);color:#fff;font-weight:700">' + esc(((p.displayName || p.handle || 'J')[0] || 'J').toUpperCase()) + '</span>';
    ...
    menu.style.cssText = 'position:absolute;top:calc(100% + 10px);right:0;min-width:190px;background:#0F1117;border:1px solid rgba(217,45,63,.5);border-radius:12px;padding:.45rem;box-shadow:0 14px 44px rgba(0,0,0,.55);display:none;z-index:9999';
    const item = (icon, label, action) => {
      ...
      a.addEventListener('mouseenter', () => { a.style.background = 'rgba(217,45,63,.18)'; });
