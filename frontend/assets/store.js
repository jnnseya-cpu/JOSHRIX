...
(function () {
  ...
  addEventListener('DOMContentLoaded', () => {
    ...
    const face = state.avatar
      ...
      : '<span style="width:30px;height:30px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--violet),var(--cyan));color:#fff;font-weight:700">' + ((state.displayName || 'J')[0] || 'J').toUpperCase() + '</span>';
    a.innerHTML = face + '<span class="jx-nav-name">' + (state.displayName || state.handle || '') + '</span>';
